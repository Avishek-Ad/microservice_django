  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      try {
        ...
        const url = `http://localhost:8080/api/v1/user/jobs/search/?q=${encodeURIComponent(searchQuery)}&department=${encodeURIComponent(department)}`;
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        ...
        <header className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Careers Portal</h1>
          <p className="mt-2 text-sm text-gray-600">
            Explore active openings across our departments and launch your application instantly.
          </p>
        </header>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Filter Department</label>
            <select
              value={department}
        ...
        {loading ? (
          <div className="flex justify-center items-center py-24">
            <p className="text-lg font-medium text-gray-600 animate-pulse">Scanning open roles...</p>
          </div>
        ) : error ? (
          ...
          <div className="bg-white text-center py-12 px-4 rounded-xl shadow-sm border border-gray-200">
            <p className="text-gray-500">No career opportunities found matching your criteria.</p>
          </div>
        ) : (
            ...
            {jobs.map((job) => (
              ...
              >
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-gray-600 line-clamp-2 max-w-2xl">{job.description}</p>
                </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl border border-gray-200 w-full max-w-md overflow-hidden transition-all animate-in fade-in zoom-in-95 duration-150">
              <div className="px-5 py-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                <div>
                  <h3 className="font-bold text-gray-900">Apply Position</h3>
                  <p className="text-xs text-gray-500 truncate max-w-[280px]">{selectedJob.title}</p>
                </div>
                <button
